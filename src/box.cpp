...
Box::Box(const Vector &c, Texture* t, double ya, double pi,
         double ro, double tx, double ty):
...
bool Box::getLightIntersection(Ray ray, double* fill){
   ...
   if( ((dist.x>=0)?dist.x:-dist.x)>textureX/2 ||
       ((dist.y>=0)?dist.y:-dist.y)>textureY/2 ) return false;

   if(texture->opacity>1-1E-6) return true;
   unsigned char temp[4];
   double amb, op, ref;

   texture->getColor(temp, &amb, &op, &ref,dist.x/textureX+.5, dist.y/textureY+.5);
